...
A session must remain bound to the authenticated user, expire when policy requires, and lose privilege after security-sensitive state changes.

## Threat Model

Review identity proof, session creation, renewal, revocation, concurrent sessions, reauthentication, and device trust. Define attacker role, controlled victim role, trusted component, protected object, and authoritative state before changing traffic. Client-side restrictions, opaque identifiers, and unusual responses are signals, not security boundaries.
...
```http
GET /api/authentication-session-management/objects/OBJECT_B HTTP/1.1
...
```http
PATCH /api/authentication-session-management/objects/OBJECT_B HTTP/1.1
Host: api.example.test
Authorization: Bearer <USER_A_TOKEN>
Content-Type: application/json
...
[Previous: Field Tips & Tricks](03-field-tips.md) · [Back to README](../README.md) · [Next: Password Reset & Account Recovery](password-reset.md)
