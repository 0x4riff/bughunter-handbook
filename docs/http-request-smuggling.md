...
Every hop must agree on request boundaries; ambiguous messages must be rejected before reaching shared connections.

## Threat Model

Review front-end/back-end message framing, connection reuse, HTTP/2 translation, and parser disagreement. Define attacker role, controlled victim role, trusted component, protected object, and authoritative state before changing traffic. Client-side restrictions, opaque identifiers, and unusual responses are signals, not security boundaries.
...
```http
GET /api/http-request-smuggling/objects/OBJECT_B HTTP/1.1
...
```http
PATCH /api/http-request-smuggling/objects/OBJECT_B HTTP/1.1
Host: api.example.test
Authorization: Bearer <USER_A_TOKEN>
Content-Type: application/json
...
[Previous: Web Cache Poisoning & Deception](web-cache.md) · [Back to README](../README.md) · [Next: Race Conditions](race-conditions.md)
