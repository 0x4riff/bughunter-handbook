...
No sequence of individually permitted identity actions should let an attacker assume another controlled identity.

## Threat Model

Review small identity weaknesses that combine across login, recovery, linking, sessions, and profile changes. Define attacker role, controlled victim role, trusted component, protected object, and authoritative state before changing traffic. Client-side restrictions, opaque identifiers, and unusual responses are signals, not security boundaries.
...
```http
GET /api/account-takeover-chains/objects/OBJECT_B HTTP/1.1
...
```http
PATCH /api/account-takeover-chains/objects/OBJECT_B HTTP/1.1
Host: api.example.test
Authorization: Bearer <USER_A_TOKEN>
Content-Type: application/json
...
[Previous: OAuth & OpenID Connect Techniques](oauth-techniques.md) · [Back to README](../README.md) · [Next: Cross-Site Scripting](xss.md)
