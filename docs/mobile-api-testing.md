...
Mobile clients are untrusted; every backend decision must remain enforced server-side regardless of app controls.

## Threat Model

Review official mobile traffic, app versions, device binding, deep links, local secrets, and backend authorization. Define attacker role, controlled victim role, trusted component, protected object, and authoritative state before changing traffic. Client-side restrictions, opaque identifiers, and unusual responses are signals, not security boundaries.
...
```http
GET /api/mobile-api-testing/objects/OBJECT_B HTTP/1.1
...
```http
PATCH /api/mobile-api-testing/objects/OBJECT_B HTTP/1.1
Host: api.example.test
Authorization: Bearer <USER_A_TOKEN>
Content-Type: application/json
...
[Previous: API Testing Techniques](api-techniques.md) · [Back to README](../README.md) · [Next: GraphQL](graphql.md)
