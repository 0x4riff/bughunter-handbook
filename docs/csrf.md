...
A cross-site origin must not cause an authenticated browser to perform an unintended state change.

## Threat Model

Review state-changing browser requests, cookie authentication, anti-CSRF tokens, SameSite behavior, and origin validation. Define attacker role, controlled victim role, trusted component, protected object, and authoritative state before changing traffic. Client-side restrictions, opaque identifiers, and unusual responses are signals, not security boundaries.
...
```http
GET /api/csrf/objects/OBJECT_B HTTP/1.1
...
```http
PATCH /api/csrf/objects/OBJECT_B HTTP/1.1
Host: api.example.test
Authorization: Bearer <USER_A_TOKEN>
Content-Type: application/json
...
[Previous: JSON Web Token Security](jwt.md) · [Back to README](../README.md) · [Next: Authorization and IDOR](authorization.md)
