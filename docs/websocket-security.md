...
Every connection and message must remain authenticated, origin-checked, and authorized for its object and action.

## Threat Model

Review handshake origin, authentication lifetime, message authorization, subscriptions, reconnect, and rate controls. Define attacker role, controlled victim role, trusted component, protected object, and authoritative state before changing traffic. Client-side restrictions, opaque identifiers, and unusual responses are signals, not security boundaries.
...
```http
GET /api/websocket-security/objects/OBJECT_B HTTP/1.1
...
```http
PATCH /api/websocket-security/objects/OBJECT_B HTTP/1.1
Host: api.example.test
Authorization: Bearer <USER_A_TOKEN>
Content-Type: application/json
...
[Previous: Cross-Origin Resource Sharing](cors.md) · [Back to README](../README.md) · [Next: File Upload Security](file-upload.md)
