...
A server-side fetcher must reach only allowed destinations using validated protocol, host, address, redirect, and response rules.

## Threat Model

Review webhooks, previews, imports, image fetchers, PDF generation, integrations, and redirect handling. Define attacker role, controlled victim role, trusted component, protected object, and authoritative state before changing traffic. Client-side restrictions, opaque identifiers, and unusual responses are signals, not security boundaries.
...
```http
GET /api/ssrf/objects/OBJECT_B HTTP/1.1
...
```http
PATCH /api/ssrf/objects/OBJECT_B HTTP/1.1
Host: api.example.test
Authorization: Bearer <USER_A_TOKEN>
Content-Type: application/json
...
[Previous: XML External Entity Injection](xxe.md) · [Back to README](../README.md) · [Next: Web Cache Poisoning & Deception](web-cache.md)
