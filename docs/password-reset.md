...
Recovery must prove control of the intended account without exposing account existence or reusable reset authority.

## Threat Model

Review account discovery, token creation, delivery, binding, expiry, consumption, session revocation, and support recovery. Define attacker role, controlled victim role, trusted component, protected object, and authoritative state before changing traffic. Client-side restrictions, opaque identifiers, and unusual responses are signals, not security boundaries.
...
```http
GET /api/password-reset/objects/OBJECT_B HTTP/1.1
...
```http
PATCH /api/password-reset/objects/OBJECT_B HTTP/1.1
Host: api.example.test
Authorization: Bearer <USER_A_TOKEN>
Content-Type: application/json
...
[Previous: Authentication & Session Management](authentication-session-management.md) · [Back to README](../README.md) · [Next: JSON Web Token Security](jwt.md)
