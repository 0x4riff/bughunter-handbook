...
Build inputs and outputs must come from authenticated, reviewed, reproducible, and least-privileged sources.

## Threat Model

Review dependencies, registries, build workflows, artifacts, provenance, secrets, and maintainer trust. Define attacker role, controlled victim role, trusted component, protected object, and authoritative state before changing traffic. Client-side restrictions, opaque identifiers, and unusual responses are signals, not security boundaries.
...
```http
GET /api/supply-chain-exposure/objects/OBJECT_B HTTP/1.1
...
```http
PATCH /api/supply-chain-exposure/objects/OBJECT_B HTTP/1.1
Host: api.example.test
Authorization: Bearer <USER_A_TOKEN>
Content-Type: application/json
...
[Previous: DNS & Cloud Misconfiguration](dns-cloud-misconfiguration.md) · [Back to README](../README.md) · [Next: Modern Web Techniques](modern-web-techniques.md)
