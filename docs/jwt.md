...
A token must be accepted only under the expected algorithm, key, issuer, audience, type, time, and privilege policy.

## Threat Model

Review algorithm selection, key lookup, issuer, audience, claims, expiry, revocation, and token type confusion. Define attacker role, controlled victim role, trusted component, protected object, and authoritative state before changing traffic. Client-side restrictions, opaque identifiers, and unusual responses are signals, not security boundaries.
...
```http
GET /api/jwt/objects/OBJECT_B HTTP/1.1
...
```http
PATCH /api/jwt/objects/OBJECT_B HTTP/1.1
Host: api.example.test
Authorization: Bearer <USER_A_TOKEN>
Content-Type: application/json
...
[Previous: Password Reset & Account Recovery](password-reset.md) · [Back to README](../README.md) · [Next: Cross-Site Request Forgery](csrf.md)
