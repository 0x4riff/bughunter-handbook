...
Shared caches must not store attacker-influenced responses for other users or expose one user’s private response to another.

## Threat Model

Review cache keys, normalization, personalized responses, unkeyed input, path confusion, and CDN-origin differences. Define attacker role, controlled victim role, trusted component, protected object, and authoritative state before changing traffic. Client-side restrictions, opaque identifiers, and unusual responses are signals, not security boundaries.
...
```http
GET /api/web-cache/objects/OBJECT_B HTTP/1.1
...
```http
PATCH /api/web-cache/objects/OBJECT_B HTTP/1.1
Host: api.example.test
Authorization: Bearer <USER_A_TOKEN>
Content-Type: application/json
...
[Previous: Server-Side Request Forgery](ssrf.md) · [Back to README](../README.md) · [Next: HTTP Request Smuggling](http-request-smuggling.md)
