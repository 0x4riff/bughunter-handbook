...
A delegated hostname must remain bound to an owned resource and must not be claimable by an unrelated account.

## Threat Model

Review dangling DNS, deprovisioned SaaS resources, custom-domain verification, and ownership lifecycle. Define attacker role, controlled victim role, trusted component, protected object, and authoritative state before changing traffic. Client-side restrictions, opaque identifiers, and unusual responses are signals, not security boundaries.
...
```http
GET /api/subdomain-takeover/objects/OBJECT_B HTTP/1.1
...
```http
PATCH /api/subdomain-takeover/objects/OBJECT_B HTTP/1.1
Host: api.example.test
Authorization: Bearer <USER_A_TOKEN>
Content-Type: application/json
...
[Previous: Exposed Development Files](exposed-development-files.md) · [Back to README](../README.md) · [Next: DNS & Cloud Misconfiguration](dns-cloud-misconfiguration.md)
