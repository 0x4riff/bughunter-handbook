---
title: "DNS & Cloud Misconfiguration"
difficulty: advanced
...
Cloud resources and DNS records must expose only intended services and principals through explicit least-privilege policy.

## Threat Model

Review records, storage, identity policies, metadata exposure, public services, logging, and lifecycle drift. Define attacker role, controlled victim role, trusted component, protected object, and authoritative state before changing traffic. Client-side restrictions, opaque identifiers, and unusual responses are signals, not security boundaries.
...
```http
GET /api/dns-cloud-misconfiguration/objects/OBJECT_B HTTP/1.1
...
```http
PATCH /api/dns-cloud-misconfiguration/objects/OBJECT_B HTTP/1.1
Host: api.example.test
Authorization: Bearer <USER_A_TOKEN>
Content-Type: application/json
...
[Previous: Subdomain Takeover](subdomain-takeover.md) · [Back to README](../README.md) · [Next: Software Supply-Chain Exposure](supply-chain-exposure.md)
